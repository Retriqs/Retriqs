...
import { getPopularLabels, searchLabels } from '@/api/retriqs'
...
const GraphLabels = () => {
  ...
  // Initialize search history on component mount and tenant change
  useEffect(() => {
    let isActive = true
  ...
  }, [selectedTenantId, bumpDropdownData])
  ...
  const reloadPopularLabels = useCallback(async () => {
    ...
    try {
      const popularLabels = await getPopularLabels(popularLabelsDefaultLimit, selectedTenantId ?? undefined)
      SearchHistoryManager.clearHistory()
  ...
  const fetchData = useCallback(
    async (query?: string): Promise<string[]> => {
      ...
      if (!query || query.trim() === '' || query.trim() === '*') {
        ...
        try {
          const apiResults = await searchLabels(query.trim(), searchLabelsDefaultLimit, selectedTenantId ?? undefined)
    ...
    [refreshTrigger, selectedTenantId] // Intentionally added to trigger re-creation when data changes
  )
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Button
        ...
        className="h-10 w-10 shrink-0 rounded-xl bg-background/95 shadow-md backdrop-blur-sm transition-all hover:bg-accent focus-visible:ring-2 focus-visible:ring-ring"
      ...
      <div className="w-full min-w-[280px] max-w-[500px]">
        <AsyncSelect<string>
          ...
          triggerClassName="h-10 w-full overflow-hidden rounded-xl bg-background/95 shadow-md backdrop-blur-sm transition-all focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2"
          searchInputClassName="h-10 border-0 focus:ring-0"
