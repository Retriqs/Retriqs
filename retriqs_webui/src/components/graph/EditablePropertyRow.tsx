...
import { updateEntity, updateRelation, checkEntityNameExists } from '@/api/retriqs'
  ...
  const handleSave = async (value: string, options?: { allowMerge?: boolean }) => {
    ...
    try {
      if (entityType === 'node' && entityId && nodeId) {
        ...
        if (name === 'entity_id') {
          if (!allowMerge) {
            const exists = await checkEntityNameExists(value, selectedTenantId ?? undefined)
            if (exists) {
        ...
        const response = await updateEntity(entityId, updatedData, true, allowMerge, selectedTenantId ?? undefined)
        ...
        await updateRelation(sourceId, targetId, updatedData, selectedTenantId ?? undefined)
        try {
