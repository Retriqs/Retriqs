...
import { controlButtonVariant, DEFAULT_GRAPH_MAX_NODES } from '@/lib/constants'
...
export default function Settings() {
  ...
  const setGraphMaxNodes = useCallback((nodes: number) => {
    const maxLimit = backendMaxGraphNodes || DEFAULT_GRAPH_MAX_NODES
      ...
      <Popover open={opened} onOpenChange={setOpened}>
        ...
        >
          <div className="flex flex-col gap-2">
            ...
            />
            <LabeledNumberInput
              label={`${t('graphPanel.sideBar.settings.maxNodes')} (≤ ${backendMaxGraphNodes || DEFAULT_GRAPH_MAX_NODES})`}
              min={1}
              max={backendMaxGraphNodes || DEFAULT_GRAPH_MAX_NODES}
              value={graphMaxNodes}
              defaultValue={backendMaxGraphNodes || DEFAULT_GRAPH_MAX_NODES}
