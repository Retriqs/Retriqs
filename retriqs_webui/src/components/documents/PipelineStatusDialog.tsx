...
import { getPipelineStatus, cancelPipeline, PipelineStatusResponse } from '@/api/retriqs'
...
export default function PipelineStatusDialog({
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        const data = await getPipelineStatus(storageId)
        setStatus(data)
      } catch (err) {
  ...
  const handleConfirmCancel = async () => {
    setShowCancelConfirm(false)
    try {
      const result = await cancelPipeline(storageId)
