...
export interface AsyncSelectProps<T> {
  ...
  debounceTime?: number
  /** Allow custom string values to be created */
  creatable?: boolean
  /** Custom label for the creatable option */
  customOptionLabel?: (query: string) => React.ReactNode
}
...
export function AsyncSelect<T>({
  ...
  const handleSelect = useCallback(
    (currentValue: string) => {
      ...
      const foundOption = options.find((option) => getOptionValue(option) === newValue)
      if (foundOption) {
        setSelectedOption(foundOption)
      } else if (creatable && newValue) {
        // Mock a basic option format for the custom value
        setSelectedOption({ value: newValue, label: newValue } as unknown as T)
      } else {
    ...
    [selectedValue, onChange, clearable, options, getOptionValue, creatable]
  )
    ...
    <Popover open={open} onOpenChange={handleOpenChange} modal={true}>
      <PopoverTrigger asChild>
        <Button
          ...
          side="bottom"
        >
          {value === '*' ? <div className="text-muted-foreground">{placeholder}</div> : (selectedOption ? getDisplayValue(selectedOption) : (initialValueDisplay || placeholder))}
      ...
      >
        <Command shouldFilter={false}>
          ...
          <CommandList>
            ...
            <CommandGroup>
              {creatable && searchTerm.trim() !== '' && !options.some(opt => getOptionValue(opt) === searchTerm.trim()) && (
                <CommandItem
                  key={searchTerm.trim()}
                  ...
                  className="truncate"
                >
                  {customOptionLabel ? customOptionLabel(searchTerm.trim()) : `Create "${searchTerm.trim()}"`}
                  <Check
                    className={cn(
                      'ml-auto h-3 w-3',
                      selectedValue === searchTerm.trim() ? 'opacity-100' : 'opacity-0'
                    )}
                  />
                </CommandItem>
              )}
