...
import { Message } from '@/api/retriqs'
...
export const ChatMessage = ({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl px-3.5 py-2 transition-all duration-300 relative text-[13px] leading-relaxed shadow-sm break-words w-fit",
        message.role === 'user'
          ? "bg-primary text-primary-foreground shadow-lg shadow-primary/10 rounded-tr-sm ml-auto"
          : message.isError
            ? "w-full bg-destructive/10 text-destructive ring-1 ring-destructive/30"
            : "glass-card text-foreground rounded-tl-sm shadow-lg shadow-black/5"
      )}
    >
      ...
      {message.role === 'assistant' && (isThinking || thinkingTime !== null) && (
        <div className={cn(
          ...
          <div
            className="flex items-center text-muted-foreground/60 hover:text-foreground transition-colors duration-200 text-[10px] font-black uppercase tracking-widest cursor-pointer select-none py-1"
            onClick={() => {
          ...
          >
            {isThinking ? (
              <>
                {isTabActive && <LoaderIcon className="mr-2 size-3 animate-spin" />}
            ...
            {finalThinkingContent && finalThinkingContent.trim() !== '' && <ChevronDownIcon className={cn("ml-2 size-3 shrink-0 transition-transform duration-300", isThinkingExpanded ? 'rotate-180' : '')} />}
          </div>
