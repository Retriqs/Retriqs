...
import { queryGraphs } from '@/api/retriqs'
...
const fetchGraph = async (label: string, maxDepth: number, maxNodes: number, storageId?: number | null) => {
  ...
  try {
    console.log(`Fetching graph label: ${queryLabel}, depth: ${maxDepth}, nodes: ${maxNodes}, storageId: ${storageId}`);
    rawData = await queryGraphs(queryLabel, maxDepth, maxNodes, storageId ?? undefined);
  } catch (e) {
    useBackendState.getState().setErrorMessage(errorMessage(e), 'Query Graphs Error!');
    toast.error('Failed to load graph data from the server.')
    return { rawGraph: null, is_truncated: false, hadError: true };
  }
  ...
  return { rawGraph, is_truncated: rawData.is_truncated, hadError: false }
}
...
const useLightrangeGraph = () => {
  ...
  useEffect(() => {
    ...
    state.setSigmaGraph(new UndirectedGraph())
  ...
  useEffect(() => {
    ...
    if (!isFetching && !useGraphStore.getState().graphDataFetchAttempted) {
      ...
      if (currentQueryLabel) {
        dataPromise = fetchGraph(currentQueryLabel, currentMaxQueryDepth, currentMaxNodes, selectedTenantId);
      } else {
  ...
  }, [currentTab, queryLabel, maxQueryDepth, maxNodes, isFetching, t, graphDataVersion, selectedTenantId])
  ...
  }, [nodeToExpand, sigmaGraph, rawGraph, t, selectedTenantId, maxNodes]);
