...
const GraphViewer = () => {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden premium-bg">
      <SigmaContainer
      ...
      >
        ...
        <div className="glass absolute bottom-6 left-6 flex flex-col rounded-2xl shadow-2xl p-1.5 gap-1">
          <LayoutsControl />
          <ZoomControl />
          <FullScreenControl />
          <LegendButton />
        ...
        {showLegend && (
          <div className="absolute bottom-24 right-6 z-10">
            <Legend className="glass rounded-2xl shadow-2xl" />
          </div>
        )}

        {/* <div className="absolute bottom-6 right-6 flex flex-col rounded-2xl border border-border/40 shadow-2xl">
      ...
      {(isFetching || isThemeSwitching) && (
        <div className="absolute inset-0 flex items-center justify-center glass z-10 transition-all duration-300">
          <div className="text-center glass-card p-8 rounded-3xl shadow-2xl border-none">
            <div className="mb-4 h-12 w-12 animate-spin rounded-full border-4 border-primary/20 border-t-primary mx-auto shadow-lg shadow-primary/10"></div>
            <p className="text-sm font-bold tracking-widest uppercase text-foreground/60">{isThemeSwitching ? 'Switching Theme...' : 'Loading Graph Data...'}</p>
          </div>
        </div>
      )}
    </div>
