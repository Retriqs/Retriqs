...
export default function DocumentManager() {
  ...
  const { selectedTenantId, tenants, loadTenants } = useTenant()
  const selectedTenant = useMemo(
    () => tenants.find((tenant) => tenant.id === selectedTenantId),
    [tenants, selectedTenantId]
  )
  const needsReembedding = isStorageNeedsReembedding(selectedTenant)
  const reembeddingInProgress =
    pipelineBusy &&
    (backendStatus?.current_job === 'reembedding' || backendStatus?.reembedding_busy === true)
  ...
  const [startingReembedding, setStartingReembedding] = useState(false)
  ...
  const handleStartReembedding = useCallback(async () => {
    if (!selectedTenantId) {
      toast.error('Select an instance first')
      return
    }

    setStartingReembedding(true)
    try {
      const result = await rebuildStorageEmbeddings(selectedTenantId)
      toast.success(
        `${result.message}. Chunks: ${result.counts.chunks}, Entities: ${result.counts.entities}, Relations: ${result.counts.relationships}.`
      )
      await loadTenants()
    } catch (err) {
      toast.error(`Failed to rebuild embeddings: ${errorMessage(err)}`)
    } finally {
      setStartingReembedding(false)
    }
  ...
  const handleIntelligentRefresh = useCallback(async (
    ...
    try {
      ...
      const response = await withTimeout(
        getDocumentsPaginated(request, selectedTenantId ?? undefined),
      ...
      if (response.documents.length === 0 && response.pagination.total_count > 0) {
        ...
        if (pageToFetch !== lastPage) {
          ...
          const lastPageResponse = await withTimeout(
            getDocumentsPaginated(lastPageRequest, selectedTenantId ?? undefined),
            30000,
            'Document fetch timeout'
          );
      ...
      if (isMountedRef.current) {
        ...
        if (errorClassification.shouldShowToast) {
          toast.error('Failed to load documents from the server.')
        }
  ...
  }, [statusFilter, pagination.page, pagination.page_size, sortField, sortDirection, selectedTenantId, t, updateComponentState, withTimeout, classifyError, recordFailure]);
  ...
  const scanDocuments = useCallback(async () => {
    try {
      ...
      const { status, message, track_id: _track_id } = await scanNewDocuments(selectedTenantId ?? undefined); // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  const handleManualRefresh = useCallback(async () => {
    try {
      ...
      const response = await getDocumentsPaginated(request, selectedTenantId ?? undefined);
  ...
  }, [statusFilter, pagination.page_size, sortField, sortDirection, handlePageSizeChange, selectedTenantId, t]);
  ...
  // Reset documents when tenant changes
  useEffect(() => {
    setDocs(null)
    setCurrentPageDocs([])
  ...
  useEffect(() => {
    if (currentTab === 'documents' && selectedTenantId !== null) {
      fetchPaginatedDocuments(pagination.page, pagination.page_size, statusFilter);
    }
  }, [
    currentTab,
  ...
  return (
    <Card className="!rounded-none !border-0 shadow-none bg-transparent flex flex-col h-full min-h-0 premium-bg">
      <CardHeader className="py-6 px-8 flex-none">
        <div>
          <CardTitle className="text-2xl font-black tracking-tight">{t('documentPanel.documentManager.title')}</CardTitle>
          <p className="text-muted-foreground text-sm mt-1 opacity-80">Manage your knowledge base documents and view processing status.</p>
        </div>
      </CardHeader>

      <CardContent className="flex-1 flex flex-col min-h-0 overflow-auto px-8 pb-24">
        <div className="flex justify-between items-center gap-4 mb-6">
          <div className="flex items-center gap-1.5 glass p-1 rounded-xl shadow-inner">
            <Button
              variant="ghost"
              ...
              size="sm"
              className="h-9 rounded-lg hover:bg-background shadow-sm transition-all"
            >
              <RefreshCwIcon className="mr-2 h-4 w-4" /> {t('documentPanel.documentManager.scanButton')}
            </Button>
            ...
            <Button
              ...
              size="sm"
              className={cn(
                "h-9 rounded-lg hover:bg-background shadow-sm transition-all",
                pipelineBusy && 'pipeline-busy text-primary bg-primary/10'
              )}
            >
              <ActivityIcon className="mr-2 h-4 w-4" />
              {reembeddingInProgress ? 'Re-embedding' : t('documentPanel.documentManager.pipelineStatusButton')}
            </Button>
          </div>
          ...
          <div className="flex items-center gap-2">
            {isSelectionMode && (
              <DeleteDocumentsDialog
                ...
                storageId={selectedTenantId ?? undefined}
              />
            )}
        ...
        {(needsReembedding || reembeddingInProgress) && (
          <div className={cn(
            'mb-4 rounded-xl border px-4 py-3 text-sm',
            reembeddingInProgress
              ...
              <div className="space-y-1">
                <p className="font-semibold">
                  {reembeddingInProgress
                    ...
                    : 'Embeddings are out of date'}
                </p>
                <p className="text-xs opacity-90">
                  {reembeddingInProgress
                    ? (backendStatus?.pipeline_latest_message || 'Updating vectors for existing chunks/entities/relations.')
                    : 'Search quality may be reduced until embeddings are rebuilt for this storage.'}
                </p>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => {
                  if (reembeddingInProgress) {
                    setShowPipelineStatus(true)
                  } else {
                    void handleStartReembedding()
                  }
                }}
                disabled={startingReembedding || !selectedTenantId}
                className="h-8"
              >
                {reembeddingInProgress
                  ? 'View Progress'
                  : (startingReembedding ? 'Starting...' : 'Start Re-embedding')}
              </Button>
            </div>
          </div>
        )}

        <Card className="flex-1 flex flex-col glass-card border-none rounded-2xl shadow-2xl shadow-primary/5 min-h-0 overflow-hidden">
          <CardHeader className="flex-none py-4 px-6 border-b border-border/10 bg-primary/5">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="flex items-center gap-3">
                <CardTitle className="text-sm font-semibold tracking-tight">{t('documentPanel.documentManager.uploadedTitle')}</CardTitle>
              </div>

              <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
                {/* Status Filters */}
                <div className="flex flex-wrap gap-1 p-1 glass rounded-xl" dir={i18n.dir()}>
                  <Button
                    size="sm"
                    variant="ghost"
                    ...
                    className={cn(
                      "h-8 rounded-lg text-xs font-medium px-3 transition-all",
                      statusFilter === 'all'
                  ...
                  >
                    ...
                    <span className="ml-1.5 opacity-60 text-[10px]">{statusCounts.all || documentCounts.all}</span>
                  </Button>
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      statusFilter === 'processed'
                        ...
                        : 'text-muted-foreground hover:text-green-600 dark:hover:text-green-400 hover:bg-green-50 dark:hover:bg-green-500/10'
                    )}
                  >
                    {t('documentPanel.documentManager.status.completed')}
                    <span className="ml-1.5 opacity-60 text-[10px]">{processedCount}</span>
                  </Button>
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      statusFilter === 'preprocessed'
                        ? 'bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-400 shadow-sm'
                        : 'text-muted-foreground hover:text-purple-600 dark:hover:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-500/10'
                    )}
                  >
                    {t('documentPanel.documentManager.status.preprocessed')}
                    <span className="ml-1.5 opacity-60 text-[10px]">{preprocessedCount}</span>
                  </Button>
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      statusFilter === 'processing'
                        ...
                        : 'text-muted-foreground hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/10'
                    )}
                  >
                    {t('documentPanel.documentManager.status.processing')}
                    <span className="ml-1.5 opacity-60 text-[10px]">{processingCount}</span>
                  </Button>
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      statusFilter === 'pending'
                        ...
                        : 'text-muted-foreground hover:text-yellow-600 dark:hover:text-yellow-400 hover:bg-yellow-50 dark:hover:bg-yellow-500/10'
                    )}
                  >
                    {t('documentPanel.documentManager.status.pending')}
                    <span className="ml-1.5 opacity-60 text-[10px]">{pendingCount}</span>
                  </Button>
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      statusFilter === 'failed'
                        ...
                        : 'text-muted-foreground hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10'
                    )}
                  >
                    {t('documentPanel.documentManager.status.failed')}
                    <span className="ml-1.5 opacity-60 text-[10px]">{failedCount}</span>
                  </Button>
                </div>
                ...
                <div className="flex items-center gap-2 border-l border-border/40 pl-4">
                  ...
                  <Button
                    ...
                    className="h-8 rounded-lg text-xs bg-background/50 hover:bg-background border-border/60"
                  >
                    {showFileName
          ...
          <CardContent className="flex-1 relative p-0" ref={cardContentRef}>
            ...
            {docs && (
              <div className="absolute inset-0 flex flex-col p-0">
                <div className="absolute inset-[-1px] flex flex-col p-0 border-r border-l border-border/10 rounded-b-2xl overflow-hidden">
                  <Table className="w-full">
                    <TableHeader className="sticky top-0 bg-secondary/30 z-10 shadow-sm border-b border-border/10">
                      <TableRow className="border-none hover:bg-transparent">
                        <TableHead
                          onClick={() => handleSort('id')}
                          className="cursor-pointer hover:text-foreground transition-colors select-none text-[11px] font-semibold tracking-wider uppercase text-muted-foreground h-10"
                        >
                        ...
                        <TableHead className="text-[11px] font-semibold tracking-wider uppercase text-muted-foreground h-10">{t('documentPanel.documentManager.columns.status')}</TableHead>
                        <TableHead className="text-[11px] font-semibold tracking-wider uppercase text-muted-foreground h-10">{t('documentPanel.documentManager.columns.length')}</TableHead>
                        <TableHead className="text-[11px] font-semibold tracking-wider uppercase text-muted-foreground h-10">{t('documentPanel.documentManager.columns.chunks')}</TableHead>
                        <TableHead
                        ...
                        <TableHead className="w-16 text-center text-[11px] font-semibold tracking-wider uppercase text-muted-foreground h-10">
                          {t('documentPanel.documentManager.columns.select')}
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody className="text-[13px] overflow-auto">
                      {filteredAndSortedDocs && filteredAndSortedDocs.map((doc) => (
                        <TableRow key={doc.id} className="border-b border-border/20 hover:bg-muted/30 transition-colors">
                          <TableCell className="truncate font-mono overflow-visible max-w-[250px] font-medium text-foreground/80 py-3">
                            {showFileName ? (
                              <>
                                ...
                                <div className="text-[10px] text-muted-foreground/60 mt-0.5">{doc.id}</div>
                              </>
                            ) : (
                              <div className="group relative overflow-visible tooltip-container">
                                <div className="truncate text-foreground">
                          ...
                          <TableCell>
                            <div className="group relative flex items-center overflow-visible tooltip-container">
                              {doc.status === 'processed' && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-green-100/50 dark:bg-green-500/10 text-green-700 dark:text-green-400 border border-green-200/50 dark:border-green-500/20">{t('documentPanel.documentManager.status.completed')}</span>
                              )}
                              {doc.status === 'preprocessed' && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-purple-100/50 dark:bg-purple-500/10 text-purple-700 dark:text-purple-400 border border-purple-200/50 dark:border-purple-500/20">{t('documentPanel.documentManager.status.preprocessed')}</span>
                              )}
                              {doc.status === 'processing' && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-blue-100/50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 border border-blue-200/50 dark:border-blue-500/20">{t('documentPanel.documentManager.status.processing')}</span>
                              )}
                              {doc.status === 'pending' && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-yellow-100/50 dark:bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 border border-yellow-200/50 dark:border-yellow-500/20">{t('documentPanel.documentManager.status.pending')}</span>
                              )}
                              {doc.status === 'failed' && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-red-100/50 dark:bg-red-500/10 text-red-700 dark:text-red-400 border border-red-200/50 dark:border-red-500/20">{t('documentPanel.documentManager.status.failed')}</span>
                              )}

                              {/* Icon rendering logic */}
                              {doc.error_msg ? (
                                <AlertTriangle className="ml-2 h-3.5 w-3.5 text-yellow-500" />
                              ) : (doc.metadata && Object.keys(doc.metadata).length > 0) && (
                                <Info className="ml-2 h-3.5 w-3.5 text-blue-400" />
                              )}
                              ...
                              {(doc.error_msg || (doc.metadata && Object.keys(doc.metadata).length > 0) || doc.track_id) && (
                                <div className="invisible group-hover:visible tooltip">
                                  ...
                                  {doc.metadata && Object.keys(doc.metadata).length > 0 && (
                                    <pre className="text-[10px]">{formatMetadata(doc.metadata)}</pre>
                                  )}
                                  {doc.error_msg && (
                                    <pre className="text-[10px] text-red-400">{doc.error_msg}</pre>
                                  )}
                                </div>
                              )}
                            </div>
                          </TableCell>
                          <TableCell className="text-muted-foreground/80">{doc.content_length ?? '-'}</TableCell>
                          <TableCell className="text-muted-foreground/80">{doc.chunks_count ?? '-'}</TableCell>
                          <TableCell className="truncate text-muted-foreground/80 text-[12px]">
                          ...
                          <TableCell className="text-center">
                            <Checkbox
                              ...
                              className="mx-auto border-muted-foreground/30 data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                            />
                          </TableCell>
                        </TableRow>
                      ))}
