  ...
  useEffect(() => {
    const checkAuthConfig = async () => {
      ...
      try {
        ...
        console.log('[login] Auth status evaluated on login page:', status)
        ...
        if (!status.auth_configured && status.access_token) {
          ...
          console.log('[login] Guest mode detected, storing token and redirecting to home')
          login(
            status.access_token,
            true,
            status.core_version,
        ...
        console.log('[login] Showing login page because auth is configured or no guest token was returned')
        setCheckingAuth(false)
      } catch (error) {
        console.error('Failed to check auth configuration:', error)
        // Also set checkingAuth to false in case of error
        console.error('[login] Showing login page because auth bootstrap failed')
        setCheckingAuth(false)
      }
        ...
        <CardHeader className="flex items-center justify-center space-y-2 pt-6 pb-8">
          <div className="flex flex-col items-center space-y-4">
            ...
            <div className="space-y-2 text-center">
              <h1 className="text-3xl font-bold tracking-tight">Retriqs</h1>
