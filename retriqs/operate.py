...
from retriqs.exceptions import (
...
from retriqs.kg.shared_storage import get_storage_keyed_lock
import time
...
async def kg_query(
    ...
    args_hash = compute_args_hash(
        ...
        str(query_param.conversation_history),
    )
...
async def extract_keywords_only(
    ...
    args_hash = compute_args_hash(
        ...
        text,
        language,
        str(param.conversation_history),
    )
    cached_result = await handle_cache(
    ...
    # Use last 10 messages for history_messages context (passed to LLM directly)
    history_for_llm = (param.conversation_history or [])[-10:]
    kw_system_prompt = PROMPTS["keywords_extraction_system"].format(
        language=language,
    )
    ...
    # Pass the system_prompt separately so it shows in OpenAI logs.
    # Pass conversation history as history_messages for proper multi-turn context.
    result = await use_model_func(
        kw_prompt,
        system_prompt=kw_system_prompt,
