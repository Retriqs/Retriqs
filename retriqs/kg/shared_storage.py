...
def direct_log(message, enable_output: bool = True, level: str = "DEBUG"):
    ...
    try:
        ...
        # Fallback if retriqs.utils is not available
...
def _namespace_matches_workspace(final_namespace: str, workspace: str) -> bool:
    """Return True when a shared namespace belongs to the given workspace."""
    if workspace == "":
        return ":" not in final_namespace
    return final_namespace.startswith(f"{workspace}:")


async def clear_workspace_namespaces(workspace: str | None = None) -> dict[str, int]:
    """
    Clear all shared in-memory namespace state for a workspace.

    This is needed when a storage workspace is replaced from disk, such as during
    archive import or merge operations. Without clearing the shared namespace maps,
    JSON-backed storages may skip reloading and continue serving stale in-memory data.

    Args:
        workspace: Workspace identifier. None uses the default workspace.

    Returns:
        Summary counts of removed namespace entries.
    """
    global _shared_dicts, _init_flags, _update_flags

    if workspace is None:
        workspace = get_default_workspace()

    if workspace is None:
        raise ValueError("Workspace must be set before clearing shared namespace state")

    if _shared_dicts is None or _init_flags is None or _update_flags is None:
        raise ValueError("Shared storage state is not initialized")
    ...
    async with get_internal_lock():
        shared_keys = [
            key for key in list(_shared_dicts.keys()) if _namespace_matches_workspace(key, workspace)
        ]
        for key in shared_keys:
            namespace_data = _shared_dicts.pop(key, None)
            if hasattr(namespace_data, "clear"):
                try:
                    namespace_data.clear()
                except Exception:
                    pass
            cleared["shared_dicts"] += 1

        init_keys = [
            key for key in list(_init_flags.keys()) if _namespace_matches_workspace(key, workspace)
        ]
        for key in init_keys:
            _init_flags.pop(key, None)
            cleared["init_flags"] += 1

        update_keys = [
            key for key in list(_update_flags.keys()) if _namespace_matches_workspace(key, workspace)
        ]
        for key in update_keys:
            flags = _update_flags.pop(key, None)
            if flags is not None:
                try:
                    for flag in list(flags):
                        if hasattr(flag, "value"):
                            flag.value = False
                    if hasattr(flags, "clear"):
                        flags.clear()
                except Exception:
                    pass
            cleared["update_flags"] += 1

    direct_log(
        f"Process {os.getpid()} cleared shared namespace state for workspace '{workspace}': {cleared}",
        level="INFO",
    )
    return cleared
