...
class Neo4JStorage(BaseGraphStorage):
    ...
    async def initialize(self):
        async with get_data_init_lock():
            # PRIORITY 1: Use instance-specific config from global_config (for per-storage isolation)
            URI = self.global_config.get("neo4j_uri")
            USERNAME = self.global_config.get("neo4j_username")
            PASSWORD = self.global_config.get("neo4j_password")
            
            # PRIORITY 2: Fallback to get_db_or_env (reads from global settings or environment)
            if not URI or not USERNAME or not PASSWORD:
                try:
                    from retriqs.api.config import get_db_or_env
                except ImportError:
                    # Fallback if not in API context
                    logger.warning("Could not import get_db_or_env, falling back to environment variables only")
                    def get_db_or_env(key, default, value_type=str, special_none=False):
                        return os.environ.get(key, default)
                
                # Only retrieve missing values
                if not URI:
                    URI = get_db_or_env("NEO4J_URI", config.get("neo4j", "uri", fallback="bolt://localhost:7687"))
                if not USERNAME:
                    USERNAME = get_db_or_env("NEO4J_USERNAME", config.get("neo4j", "username", fallback="neo4j"))
                if not PASSWORD:
                    PASSWORD = get_db_or_env("NEO4J_PASSWORD", config.get("neo4j", "password", fallback="neo4j"))
            ...
            logger.info(f"[{self.workspace}] Connecting to Neo4J at {URI} with username {USERNAME}")
    ...
    async def get_knowledge_graph(
        ...
        if max_nodes is None:
            max_nodes = self.global_config.get(
            ...
            max_nodes = min(
                max_nodes,
                self.global_config.get("max_graph_nodes", DEFAULT_MAX_GRAPH_NODES),
            )
    ...
    async def drop(self) -> dict[str, str]:
        ...
        try:
            async with self._driver.session(database=self._DATABASE) as session:
                ...
                # Drop schema indexes tied to this workspace label
                await self._drop_workspace_indexes(session, workspace_label)
    ...
    async def _drop_workspace_indexes(
        self, session, workspace_label: str
    ) -> None:
        """Drop schema indexes associated with a workspace label."""
        index_names_to_drop: set[str] = set()

        # Full-text index names are deterministic
        index_names_to_drop.add(self._get_fulltext_index_name(workspace_label))
        index_names_to_drop.add("entity_id_fulltext_idx")

        # Discover label-specific indexes via metadata
        try:
            ...
            for idx in indexes:
                ...
                if workspace_label in labels and "entity_id" in properties:
                    name = idx.get("name")
                    if name:
                        index_names_to_drop.add(name)
        except Exception as e:
            logger.warning(
                f"[{self.workspace}] Failed to inspect indexes for '{workspace_label}': {e}"
            )
            return

        for index_name in sorted(index_names_to_drop):
            try:
                drop_query = f"DROP INDEX {index_name} IF EXISTS"
                result = await session.run(drop_query)
                await result.consume()
                logger.info(
                    f"[{self.workspace}] Dropped index '{index_name}' for '{workspace_label}'"
                )
            except Exception as e:
                logger.warning(
                    f"[{self.workspace}] Failed to drop index '{index_name}' for '{workspace_label}': {e}"
                )
