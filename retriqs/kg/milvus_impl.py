...
import numpy as np
from retriqs.utils import logger, compute_mdhash_id
...
def _normalize_optional_setting(value: Any) -> str | None:
    if value is None:
        return None
...
def _resolve_milvus_setting(key: str, default: str | None, global_config: dict[str, Any]):
    ...
    try:
        ...
        db_value = get_db_setting_by_storage(key, storage_id)
    except Exception as e:
        logger.warning(
            f"Failed to fetch {key} from DB for storage {storage_id}: {e}"
        )
        return default

    normalized = _normalize_optional_setting(db_value)
            ...
            try:
                ...
                if self._client is None:
                    ...
                    self._client = MilvusClient(
                        uri=_resolve_milvus_setting(
                            "MILVUS_URI", default_uri, self.global_config
                        ),
                        user=_resolve_milvus_setting(
                            "MILVUS_USER", default_user, self.global_config
                        ),
                        password=_resolve_milvus_setting(
                            "MILVUS_PASSWORD", default_password, self.global_config
                        ),
                        token=os.environ.get(
                            "MILVUS_TOKEN", config.get("milvus", "token", fallback=None)
                        ),
                        db_name=_resolve_milvus_setting(
                            "MILVUS_DB_NAME", default_db_name, self.global_config
                        ),
                    )
                    logger.debug(
