...
def _get_storage_id_from_config(global_config: dict[str, Any]) -> int | None:
    if not global_config:
        return None

    storage_id = global_config.get("storage_id")
    if isinstance(storage_id, int):
        return storage_id
    if isinstance(storage_id, str) and storage_id.isdigit():
        return int(storage_id)

    workspace = global_config.get("workspace")
    if isinstance(workspace, str):
        match = re.match(r"storage_(\d+)$", workspace)
        if match:
...
def _resolve_redis_url(global_config: dict[str, Any]) -> str:
    default_url = os.environ.get(
        "REDIS_URI", config.get("redis", "uri", fallback="redis://localhost:6379")
    )
    storage_id = _get_storage_id_from_config(global_config)
    if storage_id is None:
        return default_url

    try:
        from retriqs.api.database.settings_manager import get_db_setting_by_storage

        db_url = get_db_setting_by_storage("REDIS_URI", storage_id)
    except Exception as e:
        logger.warning(
            f"Failed to fetch REDIS_URI from DB for storage {storage_id}: {e}"
        )
        return default_url
    ...
    return db_url if db_url else default_url
...
class RedisKVStorage(BaseKVStorage):
    def __post_init__(self):
        ...
        self._redis_url = _resolve_redis_url(self.global_config)
        self._pool = None
        self._redis = None
    ...
    async def get_by_ids(self, ids: list[str]) -> list[dict[str, Any] | None]:
        ...
        async with self._get_redis_connection() as redis:
            try:
                # 1. Construct the list of full keys
                keys = [f"{self.final_namespace}:{id}" for id in ids]
                
                # 2. Use mget to fetch all keys in a single call
                ...
                for result in results:
                    if result:
                        ...
                        # Maintain index alignment by appending None for missing keys
                ...
                # Return a list of Nones matching the input size to avoid breaking callers
                return [None] * len(ids)
    ...
    async def _migrate_legacy_cache_structure(self):
        ...
        """
        from retriqs.utils import generate_cache_key
...
class RedisDocStatusStorage(DocStatusStorage):
    ...
    async def get_by_ids(self, ids: list[str]) -> list[dict[str, Any] | None]:
        ...
        async with self._get_redis_connection() as redis:
            try:
                # 1. Prepare all keys in a single list
                keys = [f"{self.final_namespace}:{id}" for id in ids]
                
                # 2. Execute a single MGET command
                results = await redis.mget(keys)

                # 3. Process the results (maintaining the original order)
                for result_data in results:
                    if result_data:
                        ...
                        # Key did not exist in Redis
                        ordered_results.append(None)

            except Exception as e:
                logger.error(f"[{self.workspace}] Error in get_by_ids: {e}")
                # If the Redis call fails entirely, we return a list of Nones 
                # to match the length of the requested IDs.
                return [None] * len(ids)
