...
from retriqs.constants import DEFAULT_MAX_GRAPH_NODES
...
def create_graph_routes(rag_dependency, api_key: Optional[str] = None):
    combined_auth = get_combined_auth_dependency(api_key)
    
    # Create a new router instance for each call to ensure isolation
    router = APIRouter(tags=["graph"])
    
    # Import LightRAG for type hinting
    from retriqs import LightRAG

    @router.get("/graph/label/list", dependencies=[Depends(combined_auth)])
    async def get_graph_labels(rag: LightRAG = Depends(rag_dependency)):
        """
    ...
    async def get_knowledge_graph(
        ...
        max_nodes: int = Query(
            DEFAULT_MAX_GRAPH_NODES, description="Maximum nodes to return", ge=1
        ),
    ):
        """
    ...
    async def update_entity(request: EntityUpdateRequest, rag: LightRAG = Depends(rag_dependency)):
        """
    ...
    async def update_relation(request: RelationUpdateRequest, rag: LightRAG = Depends(rag_dependency)):
    ...
    async def create_entity(request: EntityCreateRequest, rag: LightRAG = Depends(rag_dependency)):
        """
    ...
    async def create_relation(request: RelationCreateRequest, rag: LightRAG = Depends(rag_dependency)):
        """
    ...
    async def merge_entities(request: EntityMergeRequest, rag: LightRAG = Depends(rag_dependency)):
        """
