...
def _normalize_chat_kwargs_for_gpt5(
    model: str,
    kwargs: dict[str, Any],
) -> None:
    """Normalize Chat Completions kwargs for GPT-5 family compatibility.

    GPT-5 family models accept `reasoning_effort` and have stricter sampling rules.
    This helper keeps existing behavior for non-GPT-5 models, while ensuring GPT-5
    requests default to medium reasoning effort and avoid unsupported sampling params.
    """
    normalized_model = (model or "").lower()
    if not normalized_model.startswith("gpt-5"):
        return

    # Allow callers to pass Responses-style reasoning payload while still using
    # Chat Completions (`reasoning={"effort":"medium"}`).
    if "reasoning_effort" not in kwargs and isinstance(kwargs.get("reasoning"), dict):
        reasoning_effort = kwargs["reasoning"].get("effort")
        if isinstance(reasoning_effort, str) and reasoning_effort.strip():
            kwargs["reasoning_effort"] = reasoning_effort.strip()
        kwargs.pop("reasoning", None)

    # Keep medium as the default effort unless explicitly provided.
    if "reasoning_effort" not in kwargs:
        kwargs["reasoning_effort"] = "medium"

    # GPT-5 generation controls can reject temperature/top_p. Drop only when
    # caller did not explicitly use reasoning_effort="none".
    reasoning_effort = str(kwargs.get("reasoning_effort", "")).lower().strip()
    if reasoning_effort != "none":
        for unsupported_param in ("temperature", "top_p"):
            if unsupported_param in kwargs:
                kwargs.pop(unsupported_param, None)
                logger.debug(
                    f"Removed unsupported parameter '{unsupported_param}' for model '{model}'"
                )
...
async def openai_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    _normalize_chat_kwargs_for_gpt5(api_model, kwargs)
